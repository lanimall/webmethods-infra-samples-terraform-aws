data "template_file" "commandcentral_inventory_entry" {
  count    = length(aws_instance.commandcentral)
  template = file("${path.cwd}/resources/ansible-inventory-entry.template")
  vars = {
    alias = var.commandcentral_hostname
    index =  tostring(count.index + 1)
    ip = aws_instance.commandcentral[count.index].private_ip
    dns = aws_instance.commandcentral[count.index].private_dns
  }
}
