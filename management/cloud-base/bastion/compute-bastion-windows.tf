...
resource "aws_route53_record" "bastion_windows" {
  ...
  name    = "${var.hostname_bastion_windows}${count.index + 1}.${module.global_common_base.name_prefix_long}.${module.base_network.dns_internal_apex}"
  type    = "A"
  ttl     = 300
  records = [
...
resource "aws_eip" "bastion_windows" {
  ...
  tags = merge(
    module.global_common_base.common_tags,
    {
      "Name" = "${module.global_common_base.name_prefix_long}-bastion_windows${count.index + 1}-${module.base_network.subnet_dmz[count.index].availability_zone}"
